...
public class NegativeTorqueState : RotationBaseState
{
    ...
    public NegativeTorqueState(RotationStateManager rotationStateManager, float torqueForce, Rigidbody rb, List<ThrusterEffectInfo> rotateThrusters, OnState smsOnState) : base(rotationStateManager, rotateThrusters,smsOnState)
    {
        this.rb = rb;
    ...
    public override void UpdateState(float rotateActionValue, float localT, Vector3 trans)
    {
        if (rotateActionValue != 0)
        {
            ...
            else { rotationStateManager.CurrentState = rotationStateManager.PositiveTorqueState; }
        }
        else if (localT != 0)
        {
            if (localT > 0) { rotationStateManager.CurrentState = rotationStateManager.PositiveTorqueState; }
