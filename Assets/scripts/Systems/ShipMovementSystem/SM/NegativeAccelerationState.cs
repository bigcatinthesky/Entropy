...
public class NegativeAccelerationState : BaseState
{
    ...
    public NegativeAccelerationState(bool dampen, StateManager stateManager, float accelerationForce, Rigidbody rb, List<ThrusterEffectInfo> moveThrusters) : base(stateManager, dampen, moveThrusters)
    {
    ...
    public override void UpdateState(float moveActionValue, float localV, Vector3 trans)
    {
        if (moveActionValue != 0)
        {
            ...
            else { stateManager.CurrentState = stateManager.PositiveAccelerationState; }
        }
        else if(dampen == true && localV != 0)
        {
            if (localV > 0) { stateManager.CurrentState = stateManager.PositiveAccelerationState; }
